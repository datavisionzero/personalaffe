...
// Problem is application/problem+json as pea reads it. Extension members stay
// in Extra so that a command can print what its code carries — the field errors
// on validation — without this package knowing every code.
type Problem struct {
	Type     string
	Title    string
	Status   int
...
// not JSON, or JSON without a `type` — is nil: the caller falls back to the
// status alone.
...
// Message is what pea prints to stderr: the detail when there is one, the title
