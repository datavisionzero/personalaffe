...
/// </summary>
/// <remarks>
/// Migrations only run forward and there is no downgrade path
/// (<c>docs/codebase.md</c>) — going back a version means restoring the backup
/// taken before the upgrade — which is what makes this refusal load-bearing
/// rather than tidy: it is the thing standing between a mistaken
/// <c>docker compose up</c> on a stale image and an instance quietly serving
/// reads and writes against a shape it misunderstands.
/// </remarks>
public sealed class SchemaIsNewerException(IReadOnlyList<string> migrations) : Exception(Describe(migrations))
{
    ...
    private static string Describe(IReadOnlyList<string> migrations) =>
        "This database was migrated by a newer personalaffe. It carries "
